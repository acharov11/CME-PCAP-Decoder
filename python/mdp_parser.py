...
class MDPParser:
    ...
    def initialize_template_sizes(self):
        ...
        self.templates = {
            47: {  # MDIncrementalRefreshOrderBook47
                'header_format': '!QBh',  # Network byte order (big-endian)
                'header_fields': [
                    'transact_time',         # uint64 - Tag 60 - UTC timestamp in nanoseconds
                    'match_event_indicator', # uint8 - Tag 5799 - Bitmap field
                    'no_md_entries',         # int16 - Tag 268 - Number of entries in group
                ],
                'group_format': '!QQiIBc',  # Format for each entry in repeating group
                'group_fields': [
                    'order_id',             # uint64NULL - Tag 37
                    'md_order_priority',    # uint64NULL - Tag 37707
                    'md_entry_px',          # PRICENULL9 - Tag 270
                    'md_display_qty',       # Int32NULL - Tag 37706
                    'md_update_action',     # uint8 - Tag 279 (0=New, 1=Change, 2=Delete)
                    'md_entry_type'         # char - Tag 269 (0=Bid, 1=Offer)
                ]
            }
        }
    ...
    def parse_mdp_message(self, data: bytes, offset: int) -> Tuple[Optional[Dict], int]:
        ...
        try:
            ...
            mei_flags = []
            if mei & 0x01: mei_flags.append('LastTrade')
            if mei & 0x02: mei_flags.append('LastVolume')
            if mei & 0x04: mei_flags.append('LastQuote')
            if mei & 0x08: mei_flags.append('LastStats')
            if mei & 0x10: mei_flags.append('LastImplied')
            if mei & 0x20: mei_flags.append('Recovery')
            if mei & 0x40: mei_flags.append('Reserved')
            if mei & 0x80: mei_flags.append('EndOfEvent')
            message['fields']['match_event_indicator'] = '|'.join(mei_flags) if mei_flags else 'None'

            # Move offset past header
            new_offset += header_size

            # Parse first repeating group entry
            ...
            group_data['md_entry_px'] = group_data['md_entry_px'] / 10000.0  # Price denominator
            group_data['md_update_action'] = self.md_update_action_map.get(
                group_data['md_update_action'], f"Unknown_{group_data['md_update_action']}")
            ...
            # Add group data to message
            message['fields'].update(group_data)

            return message, new_offset + group_size
            ...
            for timestamp, buf in pcap:
                ...
                try:
                    ...
                    while offset < len(udp.data):
                        ...
                        if message:
                            message['fields']['timestamp'] = timestamp
                            message['fields']['packet_number'] = packet_count
                            messages.append(message)
                            message_count += 1
