...
std::vector<std::string> CBOEParser::parse_payload(const std::vector<uint8_t>& packet_data,
    ...
    // @TODO; UPDATE WITH CBOE UDP SIZE (LOOK AT WIRESHARK, NEED TO SKIP BYTES BEFORE YOU GET TO ACTUAL DATA)
    ...
    // Parse general CBOE packet header
    CBOEPacketHeader cboe_packet_header;
    cboe_packet_header.pktSize = extract_field<uint16_t>(payload_data, offset, "pktSize");
    cboe_packet_header.deliveryFlag = extract_field<uint8_t>(payload_data, offset, "deliveryFlag");
    cboe_packet_header.numberMsgs = extract_field<uint8_t>(payload_data, offset, "numberMsgs");
    cboe_packet_header.seqNum = extract_field<uint32_t>(payload_data, offset, "seqNum");
    cboe_packet_header.sendTime = extract_field<uint32_t>(payload_data, offset, "sendTime");
    cboe_packet_header.sendTimeNS = extract_field<uint32_t>(payload_data, offset, "sendTimeNS");

    // Parse common message specific header
    CBOEMessageHeader cboe_message_header;
    cboe_message_header.msgSize = extract_field<uint16_t>(payload_data, offset, "msgSize");
    cboe_message_header.msgType = extract_field<uint16_t>(payload_data, offset, "msgType");

    const uint16_t& message_type = cboe_message_header.msgType;
    ...
    row = {
        to_string(cboe_packet_header.pktSize),
        to_string(cboe_packet_header.deliveryFlag),
        to_string(cboe_packet_header.numberMsgs),
        to_string(cboe_packet_header.seqNum),
        to_string(cboe_packet_header.sendTime),
        to_string(cboe_packet_header.sendTimeNS),
        to_string(cboe_message_header.msgSize),
...
std::vector<std::string> CBOEParser::parse_by_message_id(uint16_t messageID, const std::vector<uint8_t>& data, size_t& offset) {
    switch (messageID) {
        // Control messages...
        case 1:
...
std::vector<std::string> CBOEParser::parse_message_1_sequenceNumberReset(const std::vector<uint8_t> &data,
    size_t &offset) {
